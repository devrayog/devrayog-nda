...
export default function AdminQuestions() {
  ...
  const loadQuestions = async () => {
    if (!topicId) return;
    const { data: topic } = await supabase.from("study_topics").select("name").eq("id", topicId).single();
    if (topic) setTopicName((topic as any).name);
    const { data } = await supabase.from("topic_questions").select("*").eq("topic_id", topicId).order("created_at");
    if (data) setQuestions(data as Question[]);
  };

  useEffect(() => { if (isAdmin) loadQuestions(); }, [isAdmin, topicId]);

  const handleSave = async () => {
    if (!editing?.question || !editing?.option_a) return;
    setSaving(true);
    try {
      if ((editing as Question).id) {
        const { id, ...rest } = editing as Question;
        await supabase.from("topic_questions").update(rest).eq("id", id);
        toast({ title: "Question updated" });
      } else {
        await supabase.from("topic_questions").insert({ ...editing, topic_id: topicId } as any);
  ...
  const handleDelete = async (id: string) => {
    ...
    await supabase.from("topic_questions").delete().eq("id", id);
    ...
    <DashboardLayout>
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <Link to="/admin/topics" className="flex items-center gap-1 text-muted-foreground text-sm mb-2 hover:text-primary transition-colors">
              <ArrowLeft className="h-4 w-4" /> Back to Topics
            </Link>
            <div className="flex items-center gap-3">
              <HelpCircle className="h-8 w-8 text-primary" />
              <h1 className="font-display text-3xl text-gradient-gold">MCQs: {topicName}</h1>
            </div>
            <p className="text-muted-foreground text-sm mt-1">{questions.length} question{questions.length !== 1 ? "s" : ""}</p>
          </div>
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
              <Button onClick={() => setEditing({ ...emptyQ })} className="bg-gradient-gold text-primary-foreground font-bold">
                <Plus className="h-4 w-4 mr-2" /> Add Question
              </Button>
            </DialogTrigger>
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
              <DialogHeader><DialogTitle className="font-display text-gradient-gold">{(editing as Question)?.id ? "Edit Question" : "New Question"}</DialogTitle></DialogHeader>
          ...
          {questions.length === 0 && (
            <Card className="glass-card border-gold">
              <CardContent className="p-8 text-center">
                ...
                <p className="text-muted-foreground text-sm">Click "Add Question" to add MCQs for this topic.</p>
              </CardContent>
            </Card>
          )}
        </div>
