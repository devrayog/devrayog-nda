...
import { HelpCircle, Plus, Trash2, Save, Lock } from "lucide-react";
...
export default function AdminFAQ() {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    await supabase.from("faqs").insert({ question: form.question.trim(), answer: form.answer.trim(), category: form.category } as any);
    ...
    <DashboardLayout>
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display text-4xl text-gradient-gold">MANAGE FAQs</h1>
        </div>

        <Card className="glass-card border-gold">
          <CardContent className="p-4 space-y-3">
            <h3 className="font-bold text-sm flex items-center gap-2"><Plus className="h-4 w-4" /> Add FAQ</h3>
            <Input placeholder="Question" value={form.question} onChange={e => setForm(f => ({ ...f, question: e.target.value }))} className="bg-card border-border" />
            <Textarea placeholder="Answer" value={form.answer} onChange={e => setForm(f => ({ ...f, answer: e.target.value }))} className="bg-card border-border" rows={3} />
            <Button onClick={handleAdd} disabled={saving} className="bg-gradient-gold text-primary-foreground font-bold">
              <Save className="h-4 w-4 mr-1" /> Add FAQ
            </Button>
          </CardContent>
        </Card>
          ...
          <motion.div key={faq.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
            <Card className="glass-card border-gold">
              <CardContent className="p-4 flex items-start gap-3">
                ...
                <Button size="sm" variant="ghost" onClick={() => handleDelete(faq.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
              </CardContent>
            </Card>
          </motion.div>
        ))}
